...
(function () {
    ...
    // CSV MODIFIER TOOL
    // =============================================
    function setupCsvModifier() {
        if (!btnProcessCsv) return;

        btnProcessCsv.addEventListener('click', () => {
            const file = csvFileInput.files[0];
            const newPrefix = csvNewPrefixInput.value.trim();
            ...
            reader.onload = (e) => {
                ...
                let newCsvContent = lines[0] + '\n'; // Keep header

                for (let i = 1; i < lines.length; i++) {
                    const line = lines[i].trim();
                    if (!line) continue;

                    // Parse line respecting quotes, splitting by semicolon
                    // Standard naive split is usually enough for our generated CSVs,
                    // but let's be slightly more robust. Our CSV uses double quotes.
                    // Actually, a simple regex split for this specific Google Contacts CSV format generated by us:
                    ...
                    if (cols.length >= 17) {
                        // Col 0: First Name (has quotes inside the array if we didn't strip them)
                        // Actually, my manual parse loop above keeps the inner content including quotes if we didn't strip.
                        // Let's strip quotes if they exist around the column:
                        let firstName = cols[0];
                        if (firstName.startsWith('"') && firstName.endsWith('"')) {
                            firstName = firstName.substring(1, firstName.length - 1);
                        }

                        // Remove old prefix if exists: "OLD - NAME"
                        ...
                        // Apply new prefix
                        const newFirstName = newPrefix ? `${newPrefix} - ${cleanName}` : cleanName;
                        cols[0] = `"${newFirstName}"`;

                        // Col 16: Labels
                        cols[16] = newPrefix ? `"${newPrefix}"` : "";

                        // Rejoin with semicolon
                        newCsvContent += cols.join(';') + '\n';
                    } else {
                        // If it doesn't match expected columns, just pass it through
                        newCsvContent += line + '\n';
                    }
                }
                ...
                a.setAttribute('download', `contactos_modificados_${new Date().toISOString().slice(0, 10)}.csv`);
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);

                showToast('Archivo modificado descargado', 'success');
            };
    ...
    // DOM Elements for CSV Modifier
    const csvFileInput = document.getElementById('csv-file-input');
    const csvNewPrefixInput = document.getElementById('csv-new-prefix');
